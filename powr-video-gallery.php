<?php
    /**
     * @package POWr Video Gallery
     * @version 1.4.005
     */
    /*
    Plugin Name: POWr Video Gallery
    Plugin URI: http://www.powr.io
    Description: Showcase YouTube and Vimeo videos in a beautiful gallery!  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.4.005
    ...
    */
    ...
    class Powr_Video_Gallery extends WP_Widget{
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_video_gallery',
                             __( 'POWr Video Gallery' ),
                             array( 'description' => __( 'Video Gallery by POWr.io') )
        );
      }
      //This prints the div
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-video-gallery' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ?>
        <p>
          No need to edit here - just click the gears icon on your Video Gallery.
        </p>
        <p>
    ...
    function register_powr_video_gallery() {
      register_widget( 'Powr_Video_Gallery' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_video_gallery' );
    //Create short codes for adding plugins anywhere:
    function powr_video_gallery_shortcode( $atts ){
      if(isset($atts['label'])){
        $label = $atts['label'];
      }else{
        $label = '';
      }
      return "<div class='powr-video-gallery' label='$label'></div>";
    }
    add_shortcode( 'powr-video-gallery', 'powr_video_gallery_shortcode' );
